...
function doStuff(report, reportId) {
  ...
  for (let name in players) {
    ...
    if (player.left) {
      ...
      if (date == 'D0' || date == 'D1' || date == 'N1') {
        console.log(`${reportId} ` + `${player.role} left d0/d1/n1`.yellow)
        return
      }
    }
  }
  ...
  for (let i = 0; i < entries.length; i++) {
    let entry = entries[i]
    if (entry.type == 'TRIAL OUTCOME' && entry.time.split('.')[0] == 'D2' && entry.outcome == 'lynched') {
      let player = players[entry.accused]
      let faction = player.faction
      if (player.role == 'Jester') { faction = 'Jester' }
      lynchee = faction
      lyncheeName = player.name
      //OUTPUT[faction] = ++OUTPUT[faction] || 1
      //console.log(`${player.name} (${faction}) lynched D2`)
    }
    if (entry.type == 'SYSTEM' && entry.text.endsWith(' has won.')) {
      let winner = entry.text.replace(' has won.', '')
      OUTPUT[winner] = OUTPUT[winner] || {}
      OUTPUT[winner][lynchee] = ++OUTPUT[winner][lynchee] || 1
      console.log(`${lyncheeName} (${lynchee}) lynched D2 | ${winner} won`)
    }
  }

  matchesProcessed++
  console.log(reportId + ' ' + matchesProcessed + ' ' + matchesTotal + ' ' + JSON.stringify(OUTPUT, null, 0))
}
